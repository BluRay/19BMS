		...
		function() {
			...
			$("#tableWorkgroup").live(
					...
					function(e) {
						...
						if ($(e.target).attr("name") === "edit") {
							...
							getEditWorkgroupSelect();
							$("#edit_workgroup").val($(e.target).closest("tr").data("workgroupId"));
			...
			$("#checkall").live("click", function() {
				...
				if ($("#checkall").attr("checked") == "checked") {
					check_All_unAll("#tableWorkgroup", true);
				} else {
					check_All_unAll("#tableWorkgroup", false);
				}
	...
	$.ajax({
		...
		success : function(response) {
			...
			$.each(response.workgroupList, function(workgroup, value) {
				...
				tr.data("workgroupId", value.workgroupId);
				$("#tableWorkgroup tbody").append(tr);
			});
			$("#tableWorkgroup").show();
...
function ajaxAdd() {
	$.ajax({
		type : "get",
		dataType : "json",
		url : "workgroup!addWorkgroup.action",
		data : {
			"factoryId" : $("#new_factory").val(),
			"workshopId" : $("#new_workshop").val(),
			"workgroupId" : $("#new_workgroup").val(),
			"memo" : $("#new_memo").val()
		},
		success : function(response) {
...
function ajaxEdit(id) {
	...
	$.ajax({
		type : "get",
		dataType : "json",
		url : "workgroup!editWorkgroup.action",
		data : {
			"id":id,
			"workgroupId" : $("#edit_workgroup").val(),
			"memo" : $("#edit_memo").val()
		},
		success : function(response) {
...
function ajaxDelete() {
	var idlist="";
	var cboxlist=$("#tableWorkgroup tbody :checked");
	...
	$.ajax({
		type : "get",
		dataType : "json",
		url : "workgroup!deleteWorkgroup.action",
		data : {
			"idlist" : idlist
		},
		success : function(response) {
...
function getNewWorkgroupSelect() {
...
function getEditWorkgroupSelect() {
	getKeysSelect("WORKGROUP", "", "#edit_workgroup"); 
}
