...
$(document).ready(function () {	
	...
	$("#btnExport").click (function () {
		window.open("afterSale!exportVin.action?seach_factory=" + $('#seach_factory').val() + "&search_customer_name=" + $('#search_customer_name').val()
				+ "&search_fault_phenomenon=" + $('#search_fault_phenomenon').val()+ "&search_VIN=" + $('#search_VIN').val()
				+ "&search_bus_number=" + $('#search_bus_number').val()+ "&search_parts_name=" + $('#search_parts_name').val()
...
function ajaxQuery(targetPage,status){
	$.ajax({
	    ...
	    data: {
	    	...
	    	"search_bus_number": $('#search_bus_number').val(),
	    	"search_parts_name": $('#search_parts_name').val(),
	    ...
	    success:function(response){		    		
			...
			//	$("<td />").html(value.customer_bus_number).appendTo(tr);
				...
				$("<td />").html(value.severity_level!=''?value.severity_level:value.faultLevelName).appendTo(tr);    				
			...
			//	$("<td />").html(value.fault_mils).appendTo(tr);
				...
				if(value.status==0){
					...
					$("<td />").html("<i name='edit' class=\"fa fa-search\" style=\"cursor: pointer\"></i>").appendTo(tr);
				}else{
					$("<td />").html("<i name='edit' class=\"fa fa-search\" style=\"cursor: pointer\"></i>").appendTo(tr);
				}
	...
	$.ajax({
		...
		success: function (response) {
			if(response.data.length ==1){
				...
				$("#new_severity_level").val(problem.severity_level);
				...
				if(problem.attachment!=''){
					$("#attachment").attr("href","images/upload/aftersaleproblem/report/"+problem.attachment);
					$("#attachment").text('查看');
				}
