...
function ajaxEditConfirm(){
	if($("#edit_voltage").val().trim().length==0){
		alert('请输入额定电压！');
		return false;
	}else if(isNaN(parseFloat($("#edit_voltage").val()))){
		alert('额定电压只能为数字！');
		return false;
	}
	var conditions={};
	...
	conditions.voltage=$("#edit_voltage").val()
	$.ajax({
...
function ajaxEdit(factory_id,bus_number,order_no,voltage){
	//alert("-->" + bus_number + order_no);
	$("#edit_factory").val(factory_id);
	$("#edit_order_no").val(order_no);
	$("#edit_bus_number").val(bus_number);
	$("#edit_voltage").val(voltage=="undefined"?"":voltage);
	$("#editModal").modal("show");
}

function ajaxAdd (argument) {
		//数据验证
		if($("#new_factory").val() == '' || $("#new_order_no").val() == '' || $("#new_voltage").val() == ''){
			alert('请输入完整数据！');
			return false;
		}
		var new_all_bus = "0";
		...
		conditions.voltage=$("#new_voltage").val()
		$.ajax({
			type: "get",
			dataType: "json",
			url: "production!editBusInfo.action",
	...
	$.ajax({
	    ...
	    success:function(response){		    		
    		...
    		/*	$("<td style=\"text-align:center;padding:3px\" />").html(index+1).appendTo(tr);*/
    			...
    			$("<td style=\"text-align:center;padding:3px\" />").html(value.voltage).appendTo(tr);
    			$("<td style=\"text-align:center;padding:3px\" />").html("<button onclick = 'ajaxEdit(\"" + value.factory_id +
    					...
    					value.voltage + "\");' class='btn-link'>编辑</>").appendTo(tr);
