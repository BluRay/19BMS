...
function ajaxEditConfirm(){
	if($("#edit_bus_seats").val() == ''){
		alert('请输入完整数据！');
		return false;
	}
	$.ajax({
		url: "production!addBusSeats.action",
		dataType: "json",
		data: {
			...
			"new_bus_seats":$("#edit_bus_seats").val(),
			},
		async: false,
...
function ajaxEdit(factory_id,bus_number,order_no,bus_seats){
	//alert("-->" + bus_number + order_no);
	$("#edit_factory").val(factory_id);
	$("#edit_order_no").val(order_no);
	$("#edit_bus_number").val(bus_number);
	$("#edit_bus_seats").val(bus_seats);
	$("#editModal").modal("show");
}

function ajaxAdd (argument) {
		//数据验证
		if(($("#new_factory").val() == '')||($("#new_order_no").val() == '')||($("#new_bus_seats").val() == '')){
			alert('请输入完整数据！');
			return false;
		}
		var new_all_bus = "0";
		...
		$.ajax({
		    ...
		    data: {
				...
				"new_bus_seats":$("#new_bus_seats").val(),
			},
			async: false,
		    success:function (response) {
		    	if (response.success) {
...
function ajaxQuery(targetPage){
	$.ajax({
	    url: "production!getBusSeatsList.action",
	    dataType: "json",
		type: "get",
	    data: {
