...
function htmlToExcel(GridPanel_ID, ColumnWidth, Border,filename,worksheet) {
    try {
        if (navigator.userAgent.indexOf("MSIE") > 0) {
            ToExcel_IE(GridPanel_ID, ColumnWidth, Border,filename,worksheet);
        }
        else {
            ToExcel_FF(GridPanel_ID, Border,filename,worksheet);
        }
        ColumnWidth = null;
    } catch (e) {
        alert(e.description)
...
function ToExcel_IE(GridPanel_ID, ColumnWidth, Border,filename) {
...
var ToExcel_FF = (function() {
    ...
    return function(GridPanel_ID, Border,filename,worksheet) {
        var elDiv = document.getElementById(GridPanel_ID);
        //过滤elDiv内容
        var elDivStr = elDiv.innerHTML;
        elDivStr = replaceHtml(elDivStr, "<a", ">");
        ...
        //去除隐藏的TR样式
        elDivStr=elDivStr.replace(/style="display: none;"/g,"");
