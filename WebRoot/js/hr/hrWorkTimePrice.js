$(document).ready(function(){
	...
	function ajaxAdd (argument) {
		...
		var newSkillParameter = $("#newSkillParameter").val();
		var newPrice = $("#newPrice").val();
		var newStartDate = $("#newStartDate").val();
		var newEndDate = $("#newEndDate").val();
		
		if(($("#newStartDate").val()=="")||($("#newEndDate").val()=="")){
			alert("请输入开始日期和结束日期！");
			$("#newStartDate").focus();
			return false;
		}
	...
	/*	if(newWorkTimeType == "4"){
		...
		*/
		$.ajax({
			type: "get",
			dataType: "json",
			url: "hr!addWorkTimePrice.action",
		    data: {
...
function ajaxQuery(targetPage){		
	$.ajax({
		url: "hr!getWorkTimePriceList.action",
	    dataType: "json",
	    async: false,
	    type: "get",
	    data: {
	    ...
	    success:function(response){
	    	if(response.success) {
	    		...
	    		var count = 1
                $.each(response.data, function (index, price) {
                	var workTimeType = "";
                	...
                	if(price.type=="5")workTimeType = "节拍";
    				var tr = $("<tr />");
