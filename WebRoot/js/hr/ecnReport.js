...
$(document).ready(function() {
	...
	$("#export").click(function(){ 
		if(curTab=='工单维度'){
			htmlToExcel("workDimensionTable", "", "","技改工时统计","技改工时统计");
		}else
			htmlToExcel("peopleDimensionTable", "", "","技改工时统计","技改工时统计"); 
		return false;
	});
...
function ajaxQuery() {
	...
	var conditions = "{factory:'"
			...
			+ "',task:'" + $("#task").val() + "',ecn_document_number:'"
			+ $("#ecn_document_number").val() + "'}";
	...
	$(table).html("");
	$.ajax({
		url : "hrReport!getEcnReportData.action",
		dataType : "json",
		async : true,
		type : "get",
		data : data,
		success : function(response) {
			...
			var count=0;
			$.each(response.ecnReportData, function(index, salary) {
				var tr = $("<tr />");
				...
				if(flg==null){
					flg=salary.ecn_document_number;
					...
					if(flg==salary.ecn_document_number){
						//
					}else{
						var tr1 = $("<tr />");
						$("<td />").html('合计').appendTo(tr1);
						$("<td colspan='11'/>").html('').appendTo(tr1);
...
function ajaxQuery1() {
	...
	$.ajax({
		url : "hrReport!getEcnReportData1.action",
		dataType : "json",
		async : true,
		type : "get",
		data : data,
