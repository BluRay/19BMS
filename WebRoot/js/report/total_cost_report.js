...
function ajaxQuery(){
	...
	$.ajax({
	    url: "costReport!totalCost.action",
	    dataType: "json",
		type: "get",
	    data: {
	    ...
	    success:function(response){	
			...
			if(response.data.length<=0){
				...
				$.each(response.data, function(index, value) {
					categories.push(value.factory_name);
					
					var machinecost={};
					//xx.color = Highcharts.getOptions().colors[index];
					machinecost.y = Math.round(value.machine_cost*100)/100;
					machine_costs.push(machinecost);
					
					var othermaterialscost={};
					//xx.color = Highcharts.getOptions().colors[index];
					othermaterialscost.y = Math.round(value.other_materials_cost*100)/100;
					other_materials_costs.push(othermaterialscost);
					
					var othercost={};
					//xx.color = Highcharts.getOptions().colors[index];
					othercost.y = Math.round(value.other_cost*100)/100;
					other_costs.push(othercost);
					
					var toolingcost={};
					//xx.color = Highcharts.getOptions().colors[index];
					toolingcost.y = Math.round(value.tooling_cost*100)/100;
					tooling_costs.push(toolingcost);
					
					var fuelpowercost={};
					//xx.color = Highcharts.getOptions().colors[index];
					fuelpowercost.y = Math.round(value.fuel_power_cost*100)/100;
					fuel_power_costs.push(fuelpowercost);
					
					var laborcost={};
					//xx.color = Highcharts.getOptions().colors[index];
					laborcost.y = Math.round(value.labor_cost*100)/100;
